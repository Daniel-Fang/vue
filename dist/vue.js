...
(function (global, factory) {
  ...
  var callbacks = []; // 存储事件回调
  var pending = false; // 标记是否已经向任务队列添加了一个任务
  ...
  if (typeof Promise !== 'undefined' && isNative(Promise)) { // native Promise 存在
    ...
    timerFunc = function () { // 作用是将 flushCallBacks 添加到微任务队列中
  ...
  var Watcher = function Watcher (
    ...
    if (typeof expOrFn === 'function') {
      ...
      this.getter = parsePath(expOrFn); // parsePath 返回一个函数， 执行该函数返回路径对应vm的值 
      if (!this.getter) {
        this.getter = noop;
         warn(
  ...
  Watcher.prototype.get = function get () {
    ...
    var vm = this.vm;
    try {
      value = this.getter.call(vm, vm); // 调用 getter 方法，触发依赖收集
    } catch (e) {
      if (this.user) {
  ...
  function initMixin (Vue) {
    Vue.prototype._init = function (options) {
      ...
      initEvents(vm);
      initRender(vm);
      callHook(vm, 'beforeCreate'); // callHook 做两件事 1. 触发生命周期钩子 2. 触发祖先组件监听子组件的事件
